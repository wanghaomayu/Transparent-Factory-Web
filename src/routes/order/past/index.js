...
const Past = ({past, dispatch, form: {getFieldDecorator, validateFieldsAndScroll}}) => {
  const {table = {}, tableSize, tableCount, tablePage, modal = false, modalContent = {}} = past
  //   操作DropOption按钮操作
  const onMenuClick = (key, record) => {
    ...
    switch (key) {
      case 'update':
        ...
        dispatch({type: 'past/updateModalContent', payload: payload})
        dispatch({type: 'past/showModal', payload: 'update'})
        break
      case 'procedures':
        confirm({
          title: `跳转确认`,
  ...
  const onCreateClick = e => {
    e.preventDefault()
    dispatch({type: 'past/updateModalContent', payload: {modalTitle: '创建订单'}})
    dispatch({type: 'past/showModal', payload: 'create'})
  }
  //   模态框确定按钮
  const onModalOk = () => {
  ...
  const pagination = {
    ...
    onShowSizeChange: (current, pageSize) => {
      dispatch(
        routerRedux.push(`/order/past?page=${current}&size=${pageSize}`))
    },
    onChange: (current) => {
      dispatch(
  ...
  return (
    <div className='contest'>
      ...
      <Modal
        ...
        visible={!!modal}
        onCancel={() => dispatch({type: 'past/hideModal'})}
        onOk={onModalOk}
        key={'' + modal}
      >
...
export default connect(({app, past}) => ({app, past}))(
  Form.create()(Past))
